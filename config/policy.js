...
module.exports = {
	policyHandler: async function(ctx, next) {
			...
			if (contentType && contentType.indexOf("multipart") >= 0) {
				if (ctx.request.files) {
					for (const fileKey in ctx.request.files) {
						if (Object.hasOwn(ctx.request.files, fileKey)) {
							const fileObj = ctx.request.files[fileKey];
							if (fileObj instanceof Array) {
								for (let loop = 0; loop < fileObj.length; loop++) {
									fs.unlinkSync(fileObj[loop].path);
								}
							} else {
		...
		if (ctx.request && ctx.request.body && ctx.request.body.token) {
			try {
					...
					if (err.name=="TokenExpiredError") {
						try {
							if (ctx.request.body.refreshToken) {
								const decoded = jwt.verify(ctx.request.body.refreshToken, ctx.secret);
								userId = decoded._id;
								version = decoded.version;
								token = jwt.sign({_id: userId, renew: true, version: version}, ctx.secret, {expiresIn: "10h"});
								tokenExpired = true;
								ctx.request.body.tokenExpired = tokenExpired;
								ctx.request.body.token = token;
							} else {
								return ctx.ok({status: false, tokenExpired: true, msg: "Token expired"});
							}
						} catch (err) {
							return ctx.ok({status: false, tokenExpired: true, msg: "Token expired"});
						}
